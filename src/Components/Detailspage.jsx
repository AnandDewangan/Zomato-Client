...
class Detailspage extends Component {
  ...
  componentDidMount = async () => {
    ...
    const result = await axios({
      method: "GET",
      url: `https://server-lake-zeta.vercel.app/restaurants/${restaurant_id}`,
    ...
    if (cart_value) {
      if (cart_value.restaurant.name === this.state.restaurant.name) {
        this.setState({
          menuItems: cart_value.menuItems,
  ...
  handleMenuModal = async () => {
    ...
    const result = await axios({
      method: "GET",
      url: `https://server-lake-zeta.vercel.app/getmenu/${restaurant_id}`,
      headers: { "Content-Type": "application/json" },
    });
    ...
    if (
      !localValue ||
      this.state.restaurant.name != localValue.restaurant.name
    ) {
  ...
  closeMenuModal = () => {
    ...
    if (orderedItems && this.state.subTotal > 0) {
      ...
      cart_value &&
      this.state.restaurant.name === cart_value.restaurant.name
    ) {
      localStorage.removeItem("cartValue");
    }
  ...
  handleAdd = (index, operation_type) => {
    ...
    if (
      ...
      this.setState({
        handleAlert: true,
        oldCartRestaurant: cart_value.restaurant,
        cartMenuitems: filterValue,
      });
    } else {
      let total = 0;
  ...
  goToCart = () => {
    this.props.history.push(
      `/details?restaurant=${this.state.oldCartRestaurant._id}&open=${true}`
    );
  ...
  makePayment = (token) => {
    ...
    return fetch(`https://server-lake-zeta.vercel.app/payment`, {
      method: "POST",
      headers,
      ...
      .then((response) => {
        ...
        localStorage.setItem(
          "orderData",
          JSON.stringify(this.state.orderMenuItems)
        );
        localStorage.setItem(
          "restaurant",
      ...
      <div>
        ...
        <div className="container" style={{}}>
          <div className="detail-img">
            <img src={restaurant.thumb} alt="restaurant-image" width="100%"/>
          </div>
          <button className="gallery-button" onClick={this.handleGalleryModal}>
            Click to see Image Gallery
          </button>
          <div className="restaurant-name">{restaurant.name}</div>
          <button className="order-button" onClick={this.handleMenuModal}>
            Place Online Order
          </button>
          ...
          >
            ...
            <div className="address" style={{ marginBottom: "30px" }}>
              {restaurant.cuisine_id
                ? restaurant.cuisine_id.map((item) => `${item.name} `)
                : null}
            </div>
          ...
          <Modal isOpen={galleryModalIsOpen} style={customStyles}>
            ...
            <div>
              <Carousel showThumbs={false}>
                <div>
                  <img src="assets/pexels-jan-n-g-u-y-e-n-🍁-699953.jpg" className="carousel-image" />
                </div>
                <div>
                ...
                <div>
                  <img src="assets/pexels-jan-n-g-u-y-e-n-🍁-2664216.jpg" className="carousel-image" />
                </div>
                <div>
                  <img src="assets/drinks.png" className="carousel-image" />
                </div>
                <div>
                  <img src="assets/pexels-spencer-davis-4393021.jpg" className="carousel-image" />
                </div>
                <div>
                  <img src="pexels-sydney-troxell-718742.jpg" className="carousel-image" />
                </div>
              </Carousel>
            </div>
          </Modal>
          ...
          <Modal isOpen={menuModalIsOpen} style={popUpStyles}>
            <button className="carousel-button" onClick={this.closeMenuModal}>
              <span
                style={{ padding: "7px" }}
                className="glyphicon glyphicon-remove"
              ></span>
            ...
            <div className="menu-modal">
                      ...
                      >
                        ...
                        {item.qty === 0 ? (
                          <div
                            ...
                            onClick={() => this.handleAdd(index, "add")}
                          >
                            Add
                          </div>
                        ) : (
                          <div style={{ textAlign: "center" }}>
                            <span
                              ...
                              onClick={() => this.handleAdd(index, "subtract")}
                            >
                              -
                            </span>
                            <span
              ...
              <div className="Rectangle-3352">
                ...
                <span className="Rectangle-3353" onClick={this.handlePayButton}>
                  Continue
                </span>
              </div>
            </div>
          ...
          <Modal isOpen={this.state.handleAlert} style={menuStyles}>
            <div style={{ padding: "10px 0px" }}>
              <button
                className="carousel-button"
                onClick={this.closehandleAlert}
              >
                <span
                  style={{ padding: "7px" }}
                  className="glyphicon glyphicon-remove"
              ...
              <div style={{ fontSize: "14px" }}>
                Your cart has existing items from{" "}
                <strong>{this.state.oldCartRestaurant.name}</strong>. Do you
                want to clear it and add items from{" "}
              ...
              <div style={{ margin: "10px 0" }}>
                <div style={{ padding: "10px" }}>
                  ...
                  >
                    {this.state.oldCartRestaurant.name}
                  </div>
                  <div style={{ fontSize: "14px" }}>
                    {this.state.oldCartRestaurant.locality},{" "}
                    {this.state.oldCartRestaurant.city_name}
                  </div>
                </div>
                <hr />
          ...
          <Modal isOpen={this.state.paymentForm} style={menuStyles}>
            <button className="carousel-button" onClick={this.closePaymentForm}>
              <span
                style={{ padding: "7px" }}
                className="glyphicon glyphicon-remove"
              ></span>
              ...
              <div style={{ width: "400px" }}>
                ...
                <div style={{ height: "350px" }}>
                  ...
                  >
                    ...
                    <div style={{ fontSize: "14px" }}>
                      {restaurant.locality}, {restaurant.city_name}
                    </div>
                  </div>
          ...
          <Modal isOpen={this.state.isPaymentSuccess} style={menuStyles}>
            <div style={{ padding: "10px 40px" }}>
              <button
                className="carousel-button"
                onClick={this.closePaymentSuccess}
              >
                <span
                  style={{ margin: "10px" }}
                  className="glyphicon glyphicon-remove"
